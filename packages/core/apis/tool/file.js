...
import { createGetAPI, createRes, ERROR_CODES } from '../../libs/http.js';
...
export default createGetAPI(tool.filename(), async (data, context) => {
    ...
    if (!filename) {
        return createRes(ERROR_CODES.MISSING_REQUIRED_PARAMS, '文件名是必须的');
    }
    ...
    if (!exists) {
        return createRes(ERROR_CODES.FILE_NOT_FOUND, '文件未找到');
    }
    ...
    try {
        ...
        return createRes(ERROR_CODES.FILE_READ_ERROR, '读取文件信息失败', error.message);
    }
});
